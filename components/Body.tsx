...
import { PropsArtists, PropsImages, PropsTrackChoose } from '../hooks/interfaces';
...
export interface PropsSpotifyApi {
  clientId: string
  accessToken: string
  setAccessToken: (nome: string) => Promise<void>
  searchTracks: (nome: string) => Promise<void>
  getNewReleases: () => Promise<void>
  getMyRecentlyPlayedTracks: (nome: any) => Promise<void>
}

export interface IChooseTrack {
  spotifyApi: PropsSpotifyApi
  chooseTrack: (track: PropsTrackChoose) => void
}



function Body({ spotifyApi, chooseTrack }: IChooseTrack) {
  ...
  useEffect(() => {
    ...
    spotifyApi.searchTracks(search).then((res) => {
      if (cancel) return;
      setSearchResults(
        res.body.tracks.items.map((track: PropsTrack) => {
          return {
            id: track.id,
  ...
  useEffect(() => {
    ...
    spotifyApi.getNewReleases().then((res) => {
      setNewReleases(
        res.body.albums.items.map((track: PropsTrack) => {
          return {
            id: track.id,
            artist: track.artists[0].name,
            title: track.name,
  ...
  return (
    <section className="bg-black ml-24 py-4 space-y-8 md:max-w-6xl flex-grow md:mr-2.5">
     ...
     ">
        {searchResults.length === 0 ? newReleases.slice(0, 4).map((track: PropsTrackChoose) => (
          <Poster key={track.id} track={track} chooseTrack={chooseTrack} />
        )) : searchResults.slice(0, 4).map((track: PropsTrackChoose) => (
          <Poster key={track.id} track={track} chooseTrack={chooseTrack} />
        ))}
      </div>
      <div className="flex gap-x-8 absolute min-w-full md:relative ml-6">
        <div className="hidden xl:inline max-w-[270px]">
          ...
          rounded-2xl w-full font-bold bg-opacity-80 hover:bg-opacity-100 transition ease-out">
            All Genres
          </button>
        </div>
